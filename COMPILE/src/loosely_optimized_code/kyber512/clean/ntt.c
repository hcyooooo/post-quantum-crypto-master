...
#ifndef KYBER_USE_HW_NTT
#define KYBER_USE_HW_NTT 1
#endif

#if KYBER_USE_HW_NTT
/*
 * Lightweight APB3 driver for the NTT accelerator instantiated in RTL/rtl.
 * The driver mirrors the register map implemented in apb_ntt_if.sv and
 * falls back to the original software NTT when the peripheral is busy or
 * times out. Adjust HW_NTT_BASE_ADDR if the SoC memory map changes.
 */
...
#define HW_NTT_TIMEOUT  1000000u
#define HW_NTT_MIN_QINV_VALUE 0x30CFFu

static inline void hw_ntt_write(uint32_t offset, uint32_t value) {
    HW_NTT_REG(offset) = value;
}

static inline uint32_t hw_ntt_read(uint32_t offset) {
    return HW_NTT_REG(offset);
}

static int hw_ntt_wait_for_idle(void) {
    for (uint32_t iter = 0; iter < HW_NTT_TIMEOUT; ++iter) {
        uint32_t status = hw_ntt_read(HW_NTT_STATUS);
        if ((status & HW_NTT_STATUS_WRITING_BIT) == 0u) {
            if (status & HW_NTT_STATUS_CONFLICT_BIT) {
                hw_ntt_write(HW_NTT_STATUS, HW_NTT_STATUS_CONFLICT_BIT);
            }
            return 0;
        }
    }
...
static void hw_ntt_load_poly(const int16_t poly[256]) {
    for (uint32_t i = 0; i < 128; ++i) {
        uint32_t packed = ((uint16_t)poly[2 * i + 1] << 16) |
                          ((uint16_t)poly[2 * i] & 0xFFFFu);
        hw_ntt_write(HW_NTT_RF_WINDOW_BASE + (i << 2), packed);
    }
}

static void hw_ntt_store_poly(int16_t poly[256]) {
    for (uint32_t i = 0; i < 128; ++i) {
        uint32_t packed = hw_ntt_read(HW_NTT_RF_WINDOW_BASE + (i << 2));
        poly[2 * i]     = (int16_t)(packed & 0xFFFFu);
        poly[2 * i + 1] = (int16_t)(packed >> 16);
    }
}

static void hw_ntt_configure(bool inverse) {
    uint32_t cfg = 0u;
    if (!inverse) {
        cfg |= 1u;        // forward NTT
        cfg |= (1u << 1); // first rounds
    }
    hw_ntt_write(HW_NTT_CONFIG, cfg);
}

static int hw_ntt_run(int16_t poly[256], bool inverse) {
    if (hw_ntt_wait_for_idle() != 0) {
        return -1;
    }

    hw_ntt_load_poly(poly);
    hw_ntt_write(HW_NTT_PARAM_N, KYBER_N);
    hw_ntt_write(HW_NTT_MODULUS, KYBER_Q);
    hw_ntt_write(HW_NTT_MIN_QINV, HW_NTT_MIN_QINV_VALUE);
    hw_ntt_configure(inverse);

    hw_ntt_write(HW_NTT_CMD, 0x1u); // trigger ntt_start

    if (hw_ntt_wait_for_idle() != 0) {
        return -1;
    }

    hw_ntt_write(HW_NTT_STATUS, HW_NTT_STATUS_BUSY_BIT);
    hw_ntt_store_poly(poly);
    return 0;
}
#endif // KYBER_USE_HW_NTT
...
static void sw_ntt_impl(int16_t poly[256]);
static void sw_invntt_impl(int16_t poly[256]);
