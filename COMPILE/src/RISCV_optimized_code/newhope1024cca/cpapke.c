...
void PQCLEAN_NEWHOPE1024CCA_CLEAN_cpapke_keypair(unsigned char *pk,
    ...
    shake256_hw(z, 2 * NEWHOPE_SYMBYTES, z, NEWHOPE_SYMBYTES);

    gen_a(&ahat, publicseed);

    PQCLEAN_NEWHOPE1024CCA_CLEAN_poly_sample(&ahat_shat, noiseseed, 0); // sample shat
    PQCLEAN_NEWHOPE1024CCA_CLEAN_poly_ntt(&ahat_shat,&shat);

    PQCLEAN_NEWHOPE1024CCA_CLEAN_poly_sample(&ahat_shat, noiseseed, 1); // sample ehat
    PQCLEAN_NEWHOPE1024CCA_CLEAN_poly_ntt(&ahat_shat,&ehat);
...
void PQCLEAN_NEWHOPE1024CCA_CLEAN_cpapke_enc(unsigned char *c,
    ...
    PQCLEAN_NEWHOPE1024CCA_CLEAN_poly_sample(&uhat, coin, 0); // sample sprime
    PQCLEAN_NEWHOPE1024CCA_CLEAN_poly_ntt(&uhat,&sprime);

    PQCLEAN_NEWHOPE1024CCA_CLEAN_poly_sample(&uhat, coin, 1); // sample eprime
    PQCLEAN_NEWHOPE1024CCA_CLEAN_poly_ntt(&uhat,&eprime);

    PQCLEAN_NEWHOPE1024CCA_CLEAN_poly_sample(&eprimeprime, coin, 2);

    PQCLEAN_NEWHOPE1024CCA_CLEAN_poly_mul_pointwise(&vprime, &bhat, &sprime); // v'=bs'

    PQCLEAN_NEWHOPE1024CCA_CLEAN_poly_invntt(&vprime,&uhat); // inv(bs')
    ...
    PQCLEAN_NEWHOPE1024CCA_CLEAN_poly_mul_pointwise(&uhat, &ahat, &sprime); // as'
    PQCLEAN_NEWHOPE1024CCA_CLEAN_poly_add(&uhat, &uhat, &eprime); // as' + e'
...
void PQCLEAN_NEWHOPE1024CCA_CLEAN_cpapke_dec(unsigned char *m,
    ...
    PQCLEAN_NEWHOPE1024CCA_CLEAN_poly_invntt(&tmp,&shat);
